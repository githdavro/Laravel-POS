                    ...
                    <div class="bg-light rounded">
                        <div class="p-4">
                            ...
                            <div class="d-flex justify-content-between mb-4">
                                ...
                                <p class="mb-0" id="subTotal">{{ $totalPrice }} Rupiah</p>
                            </div>
                            <div class="d-flex justify-content-between">
                                <h5 class="mb-0 me-4">Deli fees:</h5>
                                <div class="">
                                    <p class="mb-0">500 Rupiah</p>
                                </div>
                            </div>
                        ...
                        <div class="py-4 mb-4 border-top border-bottom d-flex justify-content-between">
                            ...
                            <p class="mb-0 pe-4" id="finalFee">{{ $totalPrice + 500 }} Rupiah</p>
                        </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            $('.btn-plus').click(function(){
                ...
                $price = $parentNode.find("#price").text().replace("Rupiah","");
                $qty = $parentNode.find('#qty').val();
                // console.log($price, $qty);
                $totalPrice = $qty * $price;
                $parentNode.find('#eachTotal').html( $totalPrice + 'Rupiah' );
                finalCalculation();
        ...
        $('#proceedCheckOut').click(function(){
            ...
            $userId = $(".userId").val() * 1;
            // $totalPrice = $('#finalFee').text().replace("Rupiah","") * 1;


            $(" #dataTable tbody tr ").each(function( item, row){
                    $productId = $(row).find('.productId').val();
                    $qty = $(row).find('#qty').val() * 1;
                    $totalPrice = $(row).find('#eachTotal').text().replace("Rupiah", "") * 1;
        ...
        function finalCalculation(){
                $totalPrice = 0;
                $(" #dataTable tbody tr ").each(function( item, row){
                    $totalPrice += Number( $(row).find("#eachTotal").text().replace("Rupiah",""));
                })

            // console.log($totalPrice);
            $("#subTotal").html( `${$totalPrice} Rupiah `)
            $("#finalFee").html( `${$totalPrice+500} Rupiah`)
